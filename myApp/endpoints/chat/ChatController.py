...
from endpoints.chat import ChatService
from endpoints.chat.ChatService import Chat, ChatCreate, ChatUpdate
...
# Delete a chat (by id)
@router.delete("/chat/id-delete/{chat_id}", response_model=Chat, tags=["Chat"])
...
@router.delete("/chat/name-delete/{chat_name}", response_model=Chat, tags=["Chat"])
...
@router.put("/chat/update/{chat_id}", response_model=Chat, tags=["Chat"])
def update_chat_by_id(chat_id: int, updated_data: ChatUpdate, db: Session = Depends(get_db)):
